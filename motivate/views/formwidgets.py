...
class Form(tk.LabelFrame):
    ...
    ###HomeForm doesn't need these and it wouldn't work
...
class HomeForm(Form):
    ...
    def __init__(self, master, *args, **kwargs):
        ...
        self.text.grid(row=len(self.widgets)+1, column = 4)
    def SetMoney(self, money):
        self.entries[0].delete(0,'end')
        self.entries[0].insert('end', str(money))

    def SetCount(self, count): # Is this a fishy for loop too? should I pass it each button individually? 
...
class SalaryForm(Form):
    ...
    def SetCount(self, count):
        self.eventwidget.SetCount(count)

    def get_details(self):
        return int([e.get() for e in self.entries][0])
